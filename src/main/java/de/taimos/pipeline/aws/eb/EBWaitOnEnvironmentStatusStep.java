	...
	public static class Execution extends SynchronousNonBlockingStepExecution<Void> {
		...
		protected Void run() throws Exception {
			...
			DescribeEnvironmentsRequest request = DescribeEnvironmentsRequest.builder()
				...
				.environmentNames(step.environmentName)
				.build();
			...
			while (true) {
				...
				listener.getLogger().format("Environment Status: %s %n", environment.status());
				if (environment.status().name().equalsIgnoreCase(step.status)) {
					return null;
				}
