	...
	public static class Execution extends SynchronousNonBlockingStepExecution<Void> {
		...
		protected Void run() throws Exception {
			...
			SwapEnvironmentCnamEsRequest request = SwapEnvironmentCnamEsRequest.builder()
					...
					.destinationEnvironmentName(step.destinationEnvironmentName).build();
		...
		private void updateEnvironmentIdsFromUrl(ElasticBeanstalkClient client) {
			...
			if (step.sourceEnvironmentCNAME != null) {
				environment = result.environments().stream()
						...
						.findFirst()
						.orElseThrow(() -> ResourceNotFoundException.builder().message(
								String.format("Environment with url %s not found", step.sourceEnvironmentCNAME)).build());

				step.sourceEnvironmentId = environment.environmentId();
				step.sourceEnvironmentName = environment.templateName();
			}
			...
			if (step.destinationEnvironmentCNAME != null) {
				environment = result.environments().stream()
						...
						.findFirst()
						.orElseThrow(() -> ResourceNotFoundException.builder().message(
								String.format("Environment with url %s not found", step.destinationEnvironmentCNAME)).build());
