...
public class ECRPullImageStep extends AbstractStepImpl {
    ...
    public ECRPullImageStep(String imageName, String tagName, String regionName) {
    ...
    public static class Execution extends AbstractStepExecutionImpl {
        ...
        @Inject
        private transient ECRPullImageStep step;
        ...
        public boolean start() throws Exception {
            ...
            new Thread("ecrPushImage") {
                ...
                public void run() {
                    try {
                        Execution.this.listener.getLogger().println("Pulling down image");
                        final String dockerPrefix = proxyEndpoint.replaceFirst("^(http://\\.|https://|\\.)", "");
                        final String fullyQualifiedDockerImage = dockerPrefix + "/" + imageName + ":" + tagName;
                        Execution.this.listener.getLogger().format("Using docker tag: %s", fullyQualifiedDockerImage);
                        dockerClient.pullImageCmd(fullyQualifiedDockerImage)
                                .withAuthConfig(authConfig)
                                .exec(new PullImageResultCallback())
                                .awaitSuccess();
                        Execution.this.listener.getLogger().format(" Docker image %s pulled down", fullyQualifiedDockerImage);
