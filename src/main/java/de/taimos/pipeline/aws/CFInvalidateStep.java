...
public class CFInvalidateStep extends Step {
	...
	public static class Execution extends SynchronousNonBlockingStepExecution<Void> {
		...
		protected Void run() throws Exception {
			...
			CloudFrontClient client = AWSClientFactory.create(CloudFrontClient.builder(), this.getContext()).build();
			...
			InvalidationBatch batch = InvalidationBatch.builder().paths(invalidationPaths).callerReference(Long.toString(System.currentTimeMillis())).build();

			CreateInvalidationRequest build = CreateInvalidationRequest.builder().distributionId(distribution).invalidationBatch(batch).build();
			String invalidationId = client.createInvalidation(build).invalidation().id();
			...
			if (waitForCompletion) {
				...
				client.waiter().waitUntilInvalidationCompleted(GetInvalidationRequest.builder().id(invalidationId).build()).matched().exception()
					.ifPresent(ex -> {
