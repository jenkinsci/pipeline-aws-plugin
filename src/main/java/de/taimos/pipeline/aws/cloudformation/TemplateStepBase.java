...
public abstract class TemplateStepBase extends Step implements ParameterProvider {
	...
	protected RollbackConfiguration getRollbackConfiguration() {
        return RollbackConfiguration.builder().monitoringTimeInMinutes(
            getRollbackTimeoutInMinutes() == null ? 0 : getRollbackTimeoutInMinutes())
			.rollbackTriggers(parseRollbackTriggers(getRollbackTriggers())).build();
	}
