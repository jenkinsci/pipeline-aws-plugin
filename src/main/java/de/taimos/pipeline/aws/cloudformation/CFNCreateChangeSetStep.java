...
public class CFNCreateChangeSetStep extends AbstractCFNCreateStep {
	...
	public static class Execution extends AbstractCFNCreateStep.Execution<CFNCreateChangeSetStep, List<Change>> {
		...
		private List<Change> validateChangeSet(String changeSet) {
			...
			if (ChangeSetStatus.CREATE_COMPLETE == result.status()) {
				return result.changes();
			} else if (ChangeSetStatus.FAILED == result.status() && result.statusReason().toLowerCase().contains("the submitted information didn't contain changes")) {
				return result.changes();
			} else if (ChangeSetStatus.FAILED == result.status() && result.statusReason().toLowerCase().contains("no updates are to be performed")) {
				return result.changes();
			} else {
				throw new IllegalStateException("Change set did not create successfully. status=" + result.status() + " reason=" + result.statusReason());
			}
		}
